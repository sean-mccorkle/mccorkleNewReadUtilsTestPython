package mccorkleNewReadUtilsTestPython::mccorkleNewReadUtilsTestPythonClient;
...
mccorkleNewReadUtilsTestPython::mccorkleNewReadUtilsTestPythonClient
...
sub new
{
    ...
    my $self = {
	client => mccorkleNewReadUtilsTestPython::mccorkleNewReadUtilsTestPythonClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$params is a mccorkleNewReadUtilsTestPython.FilterContigsParams
$return is a mccorkleNewReadUtilsTestPython.FilterContigsResults
FilterContigsParams is a reference to a hash where the following keys are defined:
	workspace has a value which is a mccorkleNewReadUtilsTestPython.workspace_name
	contigset_id has a value which is a mccorkleNewReadUtilsTestPython.contigset_id
...
FilterContigsResults is a reference to a hash where the following keys are defined:
	...
	new_contigset_ref has a value which is a mccorkleNewReadUtilsTestPython.ws_contigset_id
...
# Authentication: required
    ...
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "mccorkleNewReadUtilsTestPython.filter_contigs",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub status
{
    ...
    my $url = $self->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
        method => "mccorkleNewReadUtilsTestPython.status",
        params => \@args,
    });
    if ($result) {
        if ($result->is_error) {
...
sub version {
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "mccorkleNewReadUtilsTestPython.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for mccorkleNewReadUtilsTestPython::mccorkleNewReadUtilsTestPythonClient\n";
    }
    if ($sMajor == 0) {
        warn "mccorkleNewReadUtilsTestPython::mccorkleNewReadUtilsTestPythonClient version is $svr_version. API subject to change.\n";
    }
}
...
package mccorkleNewReadUtilsTestPython::mccorkleNewReadUtilsTestPythonClient::RpcClient;
